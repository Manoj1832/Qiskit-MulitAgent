...
class KeyIssue(BaseModel):
    ...
    category: str = "Bug"   # Bug, Rule Violation, Performance, Security, etc.
    labels: list[str] = Field(default_factory=list) # Correctness, Reliability, etc.
            ...
            for issue in raw_issues:
                if isinstance(issue, dict):
                    key_issues.append(KeyIssue(
                        ...
                        category=_clean_yaml_value(issue.get("category", "Bug")),
...
def _parse_int(value: Any, default: int = 0, min_val: int = 0, max_val: int = 100) -> int:
    ...
    try:
        ...
        return default
def format_review_as_markdown(result: PRReviewResult) -> str:
    """Format structured review results into a beautiful GitHub comment."""
    
    # 1. Header and Quality Score
    emoji_score = "🟢" if result.score >= 80 else "🟡" if result.score >= 50 else "🔴"
    md = f"## Code Review by SWE-Agent\n\n"
    md += f"**Quality Score:** {emoji_score} **{result.score}/100** | **Effort to Review:** {'⭐' * result.estimated_effort}\n\n"
    ...
    # 4. Action Required (Bugs, Security, High Severity)
    action_cats = ["Bug", "Security", "Rule Violation"]
    action_issues = [
        i for i in result.key_issues 
        if i.category in action_cats or i.severity in ["critical", "major"]
    ]
    recommended_issues = [i for i in result.key_issues if i not in action_issues]

    if action_issues:
        md += "#### 🚨 Action Required\n"
        for i, issue in enumerate(action_issues, 1):
            icon = category_icons.get(issue.category, "🔹")
            md += f"{i}. **{issue.issue_header}** {icon} `{issue.category}` "
            md += " ".join([f"`{l}`" for l in issue.labels]) + "\n"
            md += f"   - {issue.issue_content}\n"
            if issue.relevant_file:
                md += f"   - *File:* `{issue.relevant_file}` (L{issue.start_line}-L{issue.end_line})\n"
        md += "\n"

    # 5. Review Recommended
    if recommended_issues:
        md += "#### 💡 Review Recommended\n"
        for i, issue in enumerate(recommended_issues, 1):
            icon = category_icons.get(issue.category, "🔹")
            md += f"{i}. **{issue.issue_header}** {icon} `{issue.category}` "
            md += " ".join([f"`{l}`" for l in issue.labels]) + "\n"
            md += f"   - {issue.issue_content}\n"
    ...
    if result.security_concerns and result.security_concerns.lower() != "no":
        ...
        md += f"{result.security_concerns}\n\n"

    # 7. Footer
    md += "--- \n*Generated by SWE-Agent Multi-Agent Pipeline*"
    return md
