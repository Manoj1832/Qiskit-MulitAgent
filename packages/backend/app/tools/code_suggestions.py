...
def _parse_score(value: Any) -> int:
    ...
    try:
        ...
        return 5
def format_suggestions_as_markdown(result: CodeSuggestionsResult) -> str:
    """Format structured code suggestions into a beautiful GitHub comment."""
    if not result.suggestions:
        return "## Code Suggestions by SWE-Agent\n\nNo specific improvements identified in this set of changes."

    md = f"## Code Suggestions by SWE-Agent 💡\n\n"
    md += f"Found **{len(result.suggestions)}** opportunities for improvement.\n\n"
    ...
    for i, s in enumerate(result.suggestions, 1):
        ...
        md += f"### {i}. {s.one_sentence_summary} {label_icon}\n"
        md += f"**Category:** `{s.label}` | **Impact:** `{'⭐' * (s.score // 2 or 1)}` ({s.score}/10)\n\n"
        md += f"{s.suggestion_content}\n\n"

        if s.relevant_file and s.improved_code:
            md += f"**File:** `{s.relevant_file}`\n"
            md += f"```python\n# Recommended Change\n{s.improved_code}\n```\n\n"
