...
def format_tests_as_markdown(result: TestGenerationResult) -> str:
    """Format generated test suites into a beautiful GitHub comment."""
    if not result.test_suites:
        return "## Generated Tests by SWE-Agent\n\nNo relevant test cases could be generated for these changes."

    md = f"## Generated Tests by SWE-Agent 🧪\n\n"
    md += f"Automatically generated **{result.total_tests}** test cases across **{len(result.test_suites)}** suites.\n\n"

    for i, ts in enumerate(result.test_suites, 1):
        md += f"### {i}. Suite: `{ts.test_file_name}` ({ts.framework})\n"
        md += f"Targeting improvements in your code with direct coverage.\n\n"

        # Show a summary of cases
        for tc in ts.test_cases:
            priority_emoji = "🚨" if tc.priority == "critical" else "🟠" if tc.priority == "high" else "🔹"
            md += f"- {priority_emoji} **{tc.test_name}**: {tc.test_description}\n"

        md += "\n<details>\n<summary><b>View Generated Test Code</b></summary>\n\n"
        md += f"```python\n{ts.full_test_code}\n```\n"
        md += "</details>\n\n"
